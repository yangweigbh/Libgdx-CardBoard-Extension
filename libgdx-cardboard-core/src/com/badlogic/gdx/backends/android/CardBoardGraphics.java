...
public class CardBoardGraphics implements Graphics, CardboardView.StereoRenderer {
	...
	private int width;
	private int height;
	private AndroidApplicationBase app;
	private GL20 gl20;
	private GL30 gl30;
	...
	private View createCardBoardView(AndroidApplicationBase application) {
	...
	private void setupGL() {
		...
		Gdx.app.log(LOG_TAG, "OGL renderer: " + Gdx.gl.glGetString(GL10.GL_RENDERER));
		Gdx.app.log(LOG_TAG, "OGL vendor: " + Gdx.gl.glGetString(GL10.GL_VENDOR));
		Gdx.app.log(LOG_TAG, "OGL version: " + Gdx.gl.glGetString(GL10.GL_VERSION));
		Gdx.app.log(LOG_TAG, "OGL extensions: " + Gdx.gl.glGetString(GL10.GL_EXTENSIONS));
	}
	...
	public void onDrawEye(Eye eye) {
		...
		if (!disposed)
			((CardBoardApplicationListener) app.getApplicationListener()).onDrawEye(eye);
	}
	...
	public void onFinishFrame(Viewport viewport) {
		if (!(app.getApplicationListener() instanceof CardBoardApplicationListener))
			throw new RuntimeException("should implement CardBoardApplicationListener");
		if (!disposed)
			((CardBoardApplicationListener) app.getApplicationListener()).onFinishFrame(viewport);
	}
	...
	public void onNewFrame(HeadTransform headTransform) {
		...
		if (!(app.getApplicationListener() instanceof CardBoardApplicationListener))
			throw new RuntimeException("should implement CardBoardApplicationListener");
		if (!disposed)
			((CardBoardApplicationListener) app.getApplicationListener()).onNewFrame(headTransform);
	}
